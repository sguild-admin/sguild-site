...
import { parseSyncRecordId, resolveSquareContext, type SyncErrorResponse, type SyncSuccessResponse } from "./client.schema";
...
export async function runClientExternalSync(recordId: string): Promise<SyncSuccessResponse> {
  ...
  try {
    ...
    const syncResult = await runSquareClientSync(syncInput, squareContext);
    ...
    if (Object.keys(postSyncSnapshotPatch).length > 0) {
      await persistClientExternalSnapshots(clientExternal.recordId, postSyncSnapshotPatch);
    }
