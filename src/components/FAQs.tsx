...
export default function FAQs({ city, phoneDisplay, telHref, smsHref, requestHref, includeOcean = true, serviceAreaSummary }: Props) {
  ...
  return (
    <section id="faqs" className="mx-auto max-w-6xl px-4 pt-10 pb-0">
      ...
      <div className="mt-4 space-y-3">
        ...
        <FAQItem question={`How do I book?`}>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <a href={telHref} className="inline-flex items-center rounded-xl bg-linear-to-b from-[#1b80d0] to-[#1562bc] px-4 py-2 font-sf-pro text-[1rem] font-medium text-white shadow-[0_2px_8px_rgba(27,128,208,0.18)] transition hover:brightness-95">{phoneDisplay}</a>
            ...
            <a href={requestHref} className="inline-flex items-center rounded-xl border-[0.4px] border-slate-200/45 bg-white/90 px-4 py-2 font-sf-pro text-[1rem] font-medium text-sky-700 shadow-[0_2px_6px_rgba(15,23,42,0.08)] hover:bg-white">Check Availability</a>
          </div>
        </FAQItem>
      </div>

      <div className="mt-14 mb-14 border-b border-slate-200/80" />
    </section>
  )
}
